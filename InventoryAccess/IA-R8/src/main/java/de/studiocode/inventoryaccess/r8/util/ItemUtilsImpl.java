...
public class ItemUtilsImpl implements ItemUtils {
    ...
    @Override
    public void setDisplayName(@NotNull ItemMeta itemMeta, ComponentHolder name) {
    ...
    @Override
    public void setLore(@NotNull ItemMeta itemMeta, @NotNull List<@NotNull ComponentHolder> lore) {
        ReflectionUtils.setFieldValue(
            ReflectionRegistry.CB_CRAFT_META_ITEM_LORE_FIELD,
            itemMeta,
            lore.stream().map(ComponentHolder::toJson).collect(Collectors.toList())
        );
    }
